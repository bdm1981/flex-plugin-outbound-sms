...
export class OutboundSmsView extends React.Component {
  ...
  startNotification(fromNumber, toNumber, smsMessage) {
    ...
    console.log(
      `Phone: ${toNumber} From: ${fromNumber} Message: ${smsMessage}`
    );
    const smsURL = `${process.env.FLEX_APP_FUNCTIONS_URL}/send-sms?toNumber=${encodeURI(toNumber)}&message=${encodeURI(
      smsMessage
    )}`;
    fetch(smsURL)
      ...
      .then((result) => {
        ...
        alert(
          `SMS Message has been sent to ${toNumber}.\nMessage: ${smsMessage}`
        );
  ...
  startChat(fromNumber, toName, toNumber, toEmail, toCrmid) {
    ...
    console.log(
      `From: ${fromNumber} Name: ${toName} Number: ${toNumber} Email: ${toEmail} CRMID: ${toCrmid} Contact URI: ${this.state.WorkerUri}`
    );
    console.log(
      `Calling Twilio Function: ${process.env.FLEX_APP_FUNCTIONS_URL}/create-sms-chat`
    );
    ...
    const smsURL = `${process.env.FLEX_APP_FUNCTIONS_URL}/create-sms-chat?fromNumber=${from}&toName=${phone}&toNumber=${phone}&workerUri=${WorkerUri}`;
    fetch(smsURL)
      .then((result) => result.json())
      .then((result) => {
          ...
          <Grid container direction="column" spacing={3}>
            <Paper elevation={2}>
              <div css={smsCanvas}>
                ...
                <Grid item xs={12}>
                  <p>
                    Presse the SMS Chat if you would like to start an SMS Chat
                    Task.
                  </p>
                  <br />
                  <Button
